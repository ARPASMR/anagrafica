...
# connessione al DB
drv<-dbDriver("MySQL")
conn<-try(dbConnect(drv, user="guardone", password=as.character(Sys.getenv("MYSQL_PWD")), dbname="METEO", host=as.character(Sys.getenv("DBMETEO_IP"))))
...
conn_psql = try(dbConnect(drv_psql, user="postgres", password=as.character(Sys.getenv("PSQL_PWD")), dbname="iris_devel", host="10.10.0.19"))
if (inherits(conn_psql,"try-error")) {
  print( "ERRORE nell'apertura della connessione al DB iris_devel \n")
...
if (inherits(trunc,"try-error")) {
  print( "ERRORE nel svuotare l'anagrafica del DB iris_devel \n")
  dbDisconnect(conn_psql)
  rm(conn_psql)
  dbUnloadDriver(drv_psql)
  quit(status=1)
...
if (inherits(inserimento,"try-error")) {
  print( "ERRORE nel riempire l'anagrafica del iris_devel \n")
  dbDisconnect(conn_psql)
  rm(conn_psql)
  dbUnloadDriver(drv_psql)
  quit(status=1)
